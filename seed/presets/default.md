...
**The line against `30-people/`:** this folder holds how *we* work — rules that
apply wherever the work happens. How a *named* person or team works goes to
`30-people/`, where it stays attached to them. "Raise infra changes as a
decision record" is memory; "the platform team wants infra changes raised as a
decision record" is about that team, and filing it here loses the one thing that
makes it actionable — who it is about.
...
If it names a person or a team, it belongs here even when it reads like a rule —
`05-memory/` is for conventions that hold regardless of who is involved.
...
**The test, for the case that trips people up:** a decision records a *moment of
choosing* — there were options, one was taken, the others were not. A statement
about how the system is configured today is `10-knowledge/`, **even when it
contains the word "because"**. "We sized the pool at 20 because pgbouncer
fronts it" explains how things are; "we chose a fixed pool over dynamic sizing,
having rejected X for reason Y" records a choice. If you cannot name the option
that was rejected, it is probably knowledge.
